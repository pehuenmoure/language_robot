...
},{"./showerror":3,"./utils":6}],5:[function(require,module,exports){
module.exports = {
	languageCode : 'es',
...
},{"./jquery-3.0.0.min":2}],7:[function(require,module,exports){
...
var currentLangModule = modules['es'];

var stream1OpenTime;
var stream2OpenTime;
var stream1 = [['word', 123, 0.0],
				[' ', 123, 0.0]];

var stream2 = [['word', 123, 0.0],
				[' ', 123, 0.0]];
var time;
    ...
    if (msg.results) {
      ...
      if (msg.results[0].final){
      	for(var i = 0; i < msg.results[0].alternatives[0].timestamps.length; i++){
      		var time = stream1OpenTime + msg.results[0].alternatives[0].timestamps[i][1];
      		...
      		stream1.push([word, confidence, time]);
      	}
      }
    }
}
...
function onMessage2(msg) {
    if (msg.results) {
      ...
      if (msg.results[0].final){
      	for(var i = 0; i < msg.results[0].alternatives[0].timestamps.length; i++){
      		var time = stream2OpenTime + msg.results[0].alternatives[0].timestamps[i][1];
      		var confidence = msg.results[0].alternatives[0].word_confidence[i][1];
      		var word = msg.results[0].alternatives[0].word_confidence[i][0];
      		//console.log([word, confidence, time]);
      		stream2.push([word, confidence, time]);
      	}
      	if (msg.results[0].alternatives[0].transcript.includes(currentLangModule.keyPhrase)){
      		setTimeout(function(){
      			console.log('searching for text to translate');
      			var k = 0;
	      		while(stream2[k][0] != currentLangModule.keyWord){
	      			k ++;
	      		}

	      		var t1 = Date.now()
	      		while (k+1 >= stream2.lenght) {//&& (Date.now()-t1) < 5000){
	      		}
	      		if ((Date.now()-t1) >= 5000){
	      			window.say('Perdon no te entendi');
      			}
      			else{
		      		console.log([k+1, stream2.length]);
		      		time = stream2[k+1][2];
		      		...
		      		for (var i = 0; i < stream1.length; i++){
		      			temp = Math.abs(stream1[i][2] - time);
		      			if (temp < min){
		      				min = temp;
		      				n = i;
		      			}
		      		...
		      		var text = stream1[n][0];
		      		console.log(text);
		      		var googleTranslateKey = 'AIzaSyBlhc8-zb9oVfACQUEUa89i8-iFCbqesm0';
					var url = 'https://www.googleapis.com/language/translate/v2?key=' + googleTranslateKey + '&source=en' + '&target='+ currentLangModule.targetLang + '&q=' + text;
					url = encodeURI(url);
		      		var request = $.get(url, function(data){
	  					//console.log('success');
	  					var res = data.data.translations[0].translatedText
	  					console.log(res);
	  					window.say(currentLangModule.wrapResponse(res));
	  				}).fail(function(error) {
...
tokenGenerator.getToken(function(err, token) {
	...
	initSocket(options, 
		function(){
			stream1OpenTime = Date.now();
			console.log(stream1OpenTime/1000);
		}, 
		onListening,
		onMessage1, 
	...
	options.model = currentLangModule.watsonName;
	initSocket(options, 
		function(){
			stream2OpenTime = Date.now();
			console.log(stream2OpenTime/1000);
		}, 
		onListening,
		onMessage2, 
		function(error){console.log(error)}, 
...
},{"./Microphone":1,"./socket":4,"./spanish.js":5,"./utils":6}]},{},[7]);
